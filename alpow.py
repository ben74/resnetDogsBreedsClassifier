...
def plot(df, i='x', j='y', rotate=False, fn=False, title=False,width=6,height=6):
    if type(df)==dict:    
        df=pd.DataFrame.from_dict({'x':list(df.keys()),'y':list(df.values())}).sort_values(by='y',ascending=True)
...
def _save(
    ...
    if(zip):
        ...
        p('fs : ' + str(round(os.stat(fn + '.tgz').st_size / 1024 / 1024)) + ' //--- saved in:' + str(round(time() - a)) + 's')
...
def resume(fn='allVars'):  # restore
    ...
    p('files found:', getFile([fn + '.pickle',fn + '.tgz',fn + '.h5',fn + '.joblib']))#getFile checks for zip and unzips them ,fn + '.h5.zip',fn + '.joblib.zip', fn + '.zip'
